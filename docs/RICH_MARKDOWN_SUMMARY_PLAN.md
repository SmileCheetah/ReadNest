...
- 상태: V2 생성·렌더링 구현 완료, 구조 적응형 생성 개선 반영. 번호 보존 렌더링은 후속 Frontend 작업
...
- 권장 분량: 일반적으로 원문의 10~20% 이내를 상한으로 삼되, 원문의 번호형 논리와 핵심 대조·인과관계는 보존
...
- 모든 원문에 `핵심 주장 + 항목 + 결론` 템플릿을 강제하지 않는다.
- 주장형은 주장 → 근거 → 의미, 번호형은 원문의 번호·개수·순서, 비교형은 차이와 의미, 정보형은 사실 → 맥락 흐름을 우선한다.
- 짧은 단일 주장 글은 불필요한 heading·목록·결론 없이 2~4개 문단으로 끝낼 수 있다.
- 번호형의 큰 항목은 원문에 실제 번호형 항목이 있을 때만 `### 1. 제목`처럼 heading으로 생성한다. `1. **제목**` 형태의 목록 제목은 생성하지 않는다.
- bullet과 blockquote는 논리 압축에 도움이 될 때만 사용하며, 원문에 없는 항목 수·조언·결론·배울 점을 만들지 않는다.

### Frontend 구현 결정

- ordered list parser는 입력 Markdown의 원래 번호를 보존한다. `2. 내용`을 목록 내부 index로 재계산해 `1.`로 바꾸지 않는다.
- 기존 V2 데이터 중 정확히 `숫자. **짧은 제목**`만 있는 단독 줄은 원래 번호를 포함한 h3로 호환 렌더링한다.
- V2 상세 화면은 큰 카드 안에 문서를 가두지 않고, heading·문단·목록의 읽기 리듬이 자연스럽게 이어지도록 조정한다.
- parser 회귀 테스트는 Expo SDK 57과 호환되는 `jest ~29.7.0`, `jest-expo ~57.0.5`, `@types/jest ^29.5.14`를 사용한다.
- 회귀 테스트는 legacy 번호 제목, 원본 ordered-list marker, 3번부터 시작하는 목록과 heading·bullet·quote·bold 혼합 입력을 검증한다.

### Unwind 최종 문서 스타일

가능한 경우 첫 문단에서 `### 핵심 주장`을 먼저 제시하고, `### 주요 내용`에서
원문에 실제로 있는 항목만 항목당 1~2개의 짧은 문장으로 설명한다. 원문에 명확한
결론이 있을 때만 `### 결론`을 추가한다. 핵심 문장만 bold로 표시하고 중요한
...
## 16. 항목 설명 길이 검증 분리 (2026-09-09)

### 문제

프롬프트의 120자 목표를 builder가 강제하면 Luna가 2~8자만 초과해도
정상적인 V2 요약 전체가 V1로 fallback된다.

### 가능한 선택지

1. 120자를 계속 강제하고 V1 fallback한다.
2. 초과 문장을 기계적으로 잘라낸다.
3. 120자는 생성 품질 목표로 유지하고 builder는 비정상적인 길이만 거부한다.

### 선택한 방법

3번을 선택한다. 프롬프트의 120자 지침은 유지하되, builder의 강제 상한은
항목당 500자로 분리한다. 최종 Markdown 16,000자 상한과 기존 보안 검증은
그대로 유지한다.
