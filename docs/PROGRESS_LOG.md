...
- KoDeploy 자동 빌드에서는 앱 포트를 지정할 수 없어 Dockerfile의 `EXPOSE 3000`을 단일 포트 힌트로 유지합니다.
- Dockerfile의 `ENV PORT=3000`은 두지 않고 NestJS의 `process.env.PORT || 3000` 처리로 실행 포트를 맞춥니다.
...
### KoDeploy Kubernetes Service 422 조사

자동 빌드와 Dockerfile 빌드 모두 이미지 생성 이후 다음 오케스트레이션 오류로 중단되는 현상을 조사했습니다.

```text
Service "readnest" is invalid:
...
- 저장소에는 Kubernetes `Service` 매니페스트가 없습니다.
- API는 `process.env.PORT || 3000`을 사용하고 `0.0.0.0`에 정상 바인딩합니다.
- Dockerfile의 `ENV PORT`는 제거하고 `EXPOSE 3000`만 단일 포트 힌트로 복원했습니다.
...
1. Dockerfile 빌드를 선택하고 앱 디렉토리를 `readnest-api`로 명시합니다.
2. Dockerfile에는 `EXPOSE 3000`만 포트 힌트로 두고 `ENV PORT`는 두지 않습니다.
3. KoDeploy에 별도 앱 포트 입력란이 표시되면 `3000` 하나만 지정합니다.
4. 프론트엔드, Redis 캐시, 영속 저장소를 모두 사용하지 않음으로 설정합니다.
5. MySQL도 일시적으로 사용하지 않음으로 배포해 두 번째 포트의 출처인지 확인합니다.
6. 기존 앱의 생성 설정이 남았을 가능성을 배제하기 위해 새 KoDeploy 앱으로 재현합니다.
7. 계속 실패하면 KoDeploy에 생성된 Service의 각 포트에 고유한 `name`을 넣도록 플랫폼 수정을 요청합니다.
