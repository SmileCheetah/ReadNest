...
- KoDeploy Dockerfile 배포 화면의 `포트 3000`을 단일 배포 포트 선언으로 사용합니다.
- Dockerfile의 `EXPOSE`와 `ENV PORT`는 두지 않고 NestJS의 `process.env.PORT || 3000` 처리로 실행 포트를 맞춥니다.
...
1. Dockerfile 빌드를 선택하고 경로를 `readnest-api/Dockerfile`로 지정합니다.
2. KoDeploy 앱 포트 입력란에 `3000` 하나만 지정합니다.
3. Dockerfile에는 `EXPOSE`와 `ENV PORT`를 두지 않고 환경변수 목록의 수동 `PORT`도 제거합니다.
...
- Docker 이미지 빌드와 레지스트리 push가 정상 완료되었습니다.
