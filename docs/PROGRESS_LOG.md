...
- KoDeploy가 `EXPOSE 3000`, Dockerfile의 `ENV PORT=3000`, 배포 화면의 `PORT=3000`을 중복 포트로 해석해 Kubernetes Service 생성 시 `spec.ports[].name` 오류를 낼 수 있어 Dockerfile의 포트 힌트를 제거했습니다.
- 포트는 KoDeploy 배포 화면의 환경변수 `PORT=3000`과 NestJS의 `process.env.PORT || 3000` 처리로 단일화합니다.
