...
export function ThreadDetailScreen({
  ...
  const hasV2Markdown =
    meta?.schemaVersion === 2 && isSupportedMarkdown(meta.summaryMarkdown);
  ...
  const copy = async () => {
    if (copyState === "copying" || !v2Markdown) return;
    setCopyState("copying");
    try {
      const copyText = [v2Markdown, originalUrl ? `원문: ${originalUrl}` : null]
        .filter(Boolean)
        .join("\n\n");
  ...
  return (
    <View style={styles.root}>
      ...
      >
        ...
        <View
          style={presentation === "v2" ? styles.summaryDocument : styles.card}
        >
          ...
          {presentation === "summarizing" ? (
            ...
            <View style={styles.stateBlock}>
              ...
              <Text style={styles.stateDescription}>
                이 콘텐츠는 이전 형식으로 요약되었습니다. 제목과 문단, 강조가
                적용된 새 형식으로 다시 만들어 주세요.
              </Text>
              <Pressable
                accessibilityRole="button"
                accessibilityLabel="새 형식으로 다시 요약"
                accessibilityState={{ disabled: retryPending }}
                disabled={retryPending}
                style={styles.retryCta}
                onPress={() => void retrySummary()}
              >
                <Text style={styles.retryCtaText}>
                  {retryPending ? "요청 중…" : "새 형식으로 다시 요약"}
                </Text>
              </Pressable>
            </View>
          ) : v2Markdown ? (
            <MarkdownSummary markdown={v2Markdown} />
          ) : null}
          {originalUrl ? (
            <Pressable
              accessibilityRole="button"
        ...
        {presentation === "v2" && thread.tags.length ? (
